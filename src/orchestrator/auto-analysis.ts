 ...
 */
export function buildAnalysisPrompt(threadId: string, transcript: string): string {
  return [
    `다음은 claw Discord 에이전트가 처리한 작업 대화입니다 (thread: ${threadId}).`,
    '',
    '## 대화 기록',
    transcript,
    '',
    '---',
    '',
    '이 대화를 분석해서 시스템 개선 포인트를 찾아줘:',
    '',
    '1. **반복 패턴**: 사용자가 같은 종류의 지시를 여러 번 했거나, Claude가 알아서 처리했어야 할 것을 물어본 경우',
    '2. **개선 제안** (3개 이하): claw 코드/프롬프트에서 구체적으로 개선할 수 있는 항목과 구현 방법',
    '3. **우선순위**: 임팩트 순',
    '',
    '분석 후, 이 개선들을 지금 바로 반영할지 사용자에게 물어봐라.',
  ].join('\n');
}
